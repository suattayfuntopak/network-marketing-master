...
const ZERO_TOTALS = { users: 0, message: 0, roleplay: 0, compliance: 0 }
    ...
    <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5 space-y-4 animate-in fade-in duration-200">
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: t('statsPage.archiveStatUsers'), value: totals.users },
              { label: t('statsPage.archiveStatMessages'), value: totals.message },
              { label: t('statsPage.archiveStatCoach'), value: totals.roleplay },
              { label: t('statsPage.archiveStatCompliance'), value: totals.compliance },
            ].map(stat => (
              <div
                key={stat.label}
                  ...
                  {data.rows.map(row => {
                    ...
                    return (
                      <tr key={row.userId} className="hover:bg-[var(--bg-subtle)]/60">
                        ...
                        <td className="p-3 text-center tabular-nums text-emerald-700 dark:text-emerald-400">
                          {row.messageTotal}
                        </td>
                        <td className="p-3 text-center tabular-nums text-purple-700 dark:text-purple-400">
                          {row.roleplayTotal}
                        </td>
                        <td className="p-3 text-center tabular-nums text-red-600 dark:text-red-400">
                          {row.complianceTotal}
                        </td>
