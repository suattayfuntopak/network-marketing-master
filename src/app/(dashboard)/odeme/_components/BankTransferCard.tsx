...
export function BankTransferCard() {
  ...
  return (
    <div className="max-w-6xl mx-auto rounded-3xl border border-[var(--border)] bg-[var(--bg-card)] p-6 sm:p-8 shadow-sm">
      {/* Başlık */}
      ...
      {/* İki sütun: sol IBAN, sağ adımlar + butonlar */}
      <div className="mt-6 grid gap-5 lg:grid-cols-2 items-stretch">
        {/* Sol: IBAN kutusu */}
        <div className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--bg-subtle)] p-5">
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-3)]">
                IBAN
              </span>
              <p className="mt-1.5 font-mono text-base sm:text-lg font-bold text-[var(--text-1)] break-all select-all">
          ...
          <div className="mt-4 pt-4 flex flex-wrap gap-x-6 gap-y-1 border-t border-[var(--border)] text-xs">
        ...
        <div className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--bg-subtle)] p-5">
          ...
          <div className="mt-auto pt-5 flex flex-col sm:flex-row gap-2.5">
            ...
            <a
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] px-4 py-2.5 text-xs font-bold text-[var(--text-1)] transition hover:bg-[var(--bg-subtle)] active:scale-95"
            >
