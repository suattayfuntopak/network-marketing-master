-- 020: Sprint 3 (Faz D) — team bundle RPC + platform candidate counts

-- Returns members with funnel stats, onboarding, last activity, today's AI usage, and leader-owned candidates.
...
-- Platform admin: candidate counts per workspace (service role / admin client only in app).
CREATE OR REPLACE FUNCTION public.nmm_count_candidates_per_workspace()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT coalesce(
    jsonb_object_agg(workspace_id::text, cnt),
    '{}'::jsonb
  )
  FROM (
...
REVOKE ALL ON FUNCTION public.nmm_count_candidates_per_workspace() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.nmm_count_candidates_per_workspace() TO service_role;
