-- 021: Fix nmm_fetch_team_with_downlines role for downline members.
-- A downline owner is 'leader' in their OWN workspace row; in the sponsor's team
-- view they must appear as 'member' (Distribütör). Otherwise they are mislabeled,
-- sorted above the leader, and dropped from role==='member' filters (İstatistikler).
-- Only the queried workspace owner is the 'leader'.
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      -- Only the queried workspace owner is the leader; everyone else is a member.
