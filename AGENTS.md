...
- **Server Actions (default):** all in-app mutations/queries triggered by the UI. Co-locate as `actions.ts` in the route folder (e.g. `src/app/(dashboard)/<route>/actions.ts`). **Cross-route dashboard mutations** live in `src/app/(dashboard)/actions/` (candidates, notifications, customContent, team, workspace, …). Legacy shared entry: `_shared-actions.ts`.
